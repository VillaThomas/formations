      ...
      for tp in $(jq -r '.["'"$cours"'"].tp[]' $LIST); do
        if [ -f tp/"$tp"."$LANGUAGE".md ]; then
          ...
          docker run --rm -u root -v $PWD:/formations particule/markdown-pdf:"$DOCKER_TAG" \
            -o /formations/output-html/tp/$(basename $tp)."$LANGUAGE".pdf /formations/tp/$tp.$LANGUAGE.md /formations/tp/copyright.$LANGUAGE.md
          ...
          docker run --rm -u root -v $PWD:/formations particule/markdown-pdf:"$DOCKER_TAG" \
            -o /formations/output-html/tp/$(basename $tp)."$FALLBACK_LANGUAGE".pdf /formations/tp/$tp.$FALLBACK_LANGUAGE.md /formations/tp/copyright.$FALLBACK_LANGUAGE.md
        else
